...
try {
    ...
    if(isset($_GET['not_alumno']) && $_GET['not_alumno'] != ''){

        $alum_id = $_GET['not_alumno'];

        $alumno = new Alumno(["alum_id" => $alum_id]);
        $alumnos = $alumno->buscar();

        $Notas = new Notas(["not_alumno" => $alum_id]);
        $Nota = $Notas->buscar();
